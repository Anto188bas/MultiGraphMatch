...
public class SymmetryCondition {
    ...
    private static int[][] create_sequence_matrix(QueryStructure query, int number_of_nodes) {
        ...
        for (int i = 0; i < number_of_nodes; i++) {
            for (int j = 0; j < number_of_nodes; j++) {
                sequence[i][j] = 0;
                if (!(query.isOut(i, j) || query.isIn(i, j) || query.isRev(i, j))) continue;
                for (int k = 0; k < number_of_nodes; k++) {
                    if (!(query.isOut(j, k) || query.isIn(j, k) || query.isRev(j, k))) continue;
                    sequence[i][j] += 1;
                }
            }
    ...
    private static void isomorphicExtensions(int[] fDir, int[] fRev, ObjectArrayList<int[]> vv, BitSet support, int pos, QueryStructure query) {
        ...
        Arrays.fill(cand, -1);
        int i, j;
        if (pos == node_number) vv.add(Arrays.copyOfRange(fDir, 0, node_number));
        else {
            ...
            for (i = 0; i < node_number; i++) {
                ...
                for (int neigh : vNei) {
                    ...
                    if (count[neigh] == 0) cand[ncand++] = neigh;
                    count[neigh]++;
                }
            }
            ...
            for (i = 1; i < ncand; i++)
                if (count[i] > count[neigh_max_count]) neigh_max_count = i;
    ...
    public static IntArrayList[] getNodeSymmetryConditions(QueryStructure query) {
        ...
        for (i = 0; i < numNodes; i++) {
            for (j = 0; j < vv_size; j++) {
                if (!broken.get(j) && vv.get(j)[i] != i) break;
            }
            if (j < vv_size) {
            ...
            mapAdiacs.int2ObjectEntrySet().fastForEach(adiac -> {
                ...
                for (int i = 0; i < listEdgeIds.size(); i++) {
                    ...
                    for (int j = i + 1; j < listEdgeIds.size(); j++) {
                        ...
                        if (idEdge1 < idEdge2 && qe1.equivalent_to(qe2)) symmCond[idEdge2].add(idEdge1);
                    }
                }
            });
        });
