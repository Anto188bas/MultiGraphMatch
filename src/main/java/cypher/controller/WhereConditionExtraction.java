...
public class WhereConditionExtraction {
    ...
    public WhereConditionExtraction() {
        this.origin = new String[]{"\\s*\\|\\s*", "\\s*<>\\s*", "\\s*<=\\s*", "\\s*>=\\s*", "\\s*>\\s*", "\\s*<\\s*", "\\s*=~\\s*", "\\s*=\\s*", "\\s+IS NULL\\s+", "\\s+IS NOT NULL\\s+", "NOT\\s+", "\\s+STARTS WITH\\s+", "\\s+ENDS WITH\\s+", "\\s+CONTAINS\\s+", "\\s+IN\\s+"};
        this.replacement = new String[]{"::", "_NEQ_", "_LEQ_", "_GEQ_", "_G_", "_L_", "_MR_", "_OPEQ_", "_EQNULL_", "_NEQNULL_", "_N_", "_STARTS_WITH_", "_ENDS_WITH_", "_CONTAINS_", "_IN_"};
        this.new_origin = new String[]{"|", "!=", "<=", ">=", ">", "<", "=~", "=", "=NULL", "!=NULL", "NOT ", " StartsWith ", " EndsWith ", " Contains ", " In "};
    ...
    public void normal_form_computing() {
        ...
        formula.getLiterals().forEach(literature -> condvals[count.getAndIncrement()] = origin2custom_characters(replacement, new_origin, literature.toString()));
    ...
    // # 2
    public String getDisj_where_cond() {
        return disj_where_cond;
    }

    public void setDisj_where_cond(String disj_where_cond) {
        this.disj_where_cond = disj_where_cond;
    }
...
//        System.out.println("********************************************************************************");
    ...
    public int getNumOrProposition() {
        return numOrProposition;
    }
}
